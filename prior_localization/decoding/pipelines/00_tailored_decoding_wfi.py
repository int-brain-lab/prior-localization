...
import numpy as np
from prior_pipelines.pipelines.wfi_utils import prepare_widefield_data
...
if kwargs['run_integration_test']:
    ...
    for f in results_fit_eid:        
        ...
        for (fit_pred, fit_targ) in zip(predicteds['fit'], targets['fit']):
            np.testing.assert_allclose(fit_pred['Rsquared_test_full'], fit_targ['Rsquared_test_full'], rtol=1e-2, atol=1)
            np.testing.assert_allclose(fit_pred['predictions_test'], fit_targ['predictions_test'], rtol=5e-2, atol=1)
