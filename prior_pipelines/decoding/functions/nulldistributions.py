...
def generate_null_distribution_session(trials_df, metadata, **kwargs):
    ...
    if kwargs["use_imposter_session"]:
        if not kwargs["constrain_null_session_with_beh"]:
            ...
            feedback_0contrast = trials_df[
                (trials_df.contrastLeft == 0).values
                + (trials_df.contrastRight == 0).values
            ].feedbackType.mean()

            pseudosess_s = [
                generate_imposter_session(
                    kwargs["imposterdf"],
                    metadata["eid"],
                    trials_df.index.size,
                    nbSampledSess=10,
            ...
            feedback_pseudo_0cont = [
                pseudo[
                    (pseudo.contrastLeft == 0).values
                    + (pseudo.contrastRight == 0).values
                ].feedbackType.mean()
                for pseudo in pseudosess_s
            ]
            pseudosess = pseudosess_s[
                np.argmin(np.abs(np.array(feedback_pseudo_0cont) - feedback_0contrast))
            ]
    else:
...
def generate_imposter_session(
    ...
    if stitching_for_imposter_session:
        ...
        imposterdf["sess_size"] = imposterdf["template_sess"].map(
            imposterdf["template_sess"].value_counts().to_dict()
        )
        imposter_eids = np.random.choice(
            imposterdf[
                (imposterdf.template_sess != template_sess_eid)
                * (imposterdf["sess_size"] >= nbtrials)
            ].eid.unique(),
            size=1,
            replace=False,
        )
    ...
    sub_imposterdf["sorted_eids"] = sub_imposterdf.apply(
        lambda x: (
            np.argmax(imposter_eids == x["eid"]) * sub_imposterdf.index.size + x.row_id
        ),
        axis=1,
    )
    identical_comp = (
        np.argmax(sub_imposterdf.eid.values[None] == imposter_eids[:, None], axis=0)
        * sub_imposterdf.index.size
        + sub_imposterdf.row_id
    )
    ...
    sub_imposterdf = sub_imposterdf[
        (sub_imposterdf.probabilityLeft != 0.5)
    ...
    if pLeftChange_when_stitch and stitching_for_imposter_session:
        ...
        for i, imposter_eid in enumerate(imposter_eids):
            ...
            first_pLeft = sub_imposterdf[
                (sub_imposterdf.eid == imposter_eid)
            ].probabilityLeft.values[0]

            if np.abs(first_pLeft - current_last_pLeft) < 1e-8:
                first_pLeft_idx = sub_imposterdf[
                    sub_imposterdf.eid == imposter_eid
                ].index[0]
                second_pLeft_idx = sub_imposterdf[
                    (sub_imposterdf.eid == imposter_eid)
                    & (sub_imposterdf.probabilityLeft.values != first_pLeft)
                ].index[0]
                sub_imposterdf = sub_imposterdf.drop(
                    np.arange(first_pLeft_idx, second_pLeft_idx)
                )
                first_pLeft = sub_imposterdf[
                    (sub_imposterdf.eid == imposter_eid)
                ].probabilityLeft.values[0]
            ...
            if np.abs(first_pLeft - current_last_pLeft) > 1e-8:
                valid_imposter_eids.append(
                    imposter_eid
                )  # if first pLeft is different from current pLeft, accept sess
                ...
                second2last_pLeft = (
                    ...
                    - sub_imposterdf[
                        (sub_imposterdf.eid == imposter_eid)
                    ].probabilityLeft.values[-1]
                )
                second2last_block_idx = sub_imposterdf[
                    (sub_imposterdf.eid == imposter_eid)
                    & (
                        np.abs(sub_imposterdf.probabilityLeft - second2last_pLeft)
                        < 1e-8
                    )
                ].index[-1]
                last_block_idx = sub_imposterdf[
                    (sub_imposterdf.eid == imposter_eid)
                ].index[-1]
                sub_imposterdf = sub_imposterdf.drop(
        ...
        sub_imposterdf = sub_imposterdf[
            sub_imposterdf.eid.isin(valid_imposter_eids)
        ].sort_values(by=["sorted_eids"])
