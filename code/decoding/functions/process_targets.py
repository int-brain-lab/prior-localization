...
from code.pipelines.utils_common_pipelines import load_behavior
...
def optimal_Bayesian(act, side):
    ...
    l = torch.cat(
        (
            ...
            torch.cat(
                (torch.diag(1 - hazard[:-1]), torch.zeros(nb_blocklengths - 1)[None]),
                axis=0,
            ),
        ),
        axis=-1,
    ...
    lks = torch.hstack(
        [
            ...
            torch.ones_like(act[:, None]) * 1.0 / 2,
            gamma * (side[:, None] == 1) + (1 - gamma) * (side[:, None] == -1),
        ]
    )
...
def compute_beh_target(trials_df, metadata, remove_old=False, **kwargs):
    ...
    if kwargs["target"] in ["signcont", "strengthcont"]:
        if "signedContrast" in trials_df.keys():
            ...
            out = np.nan_to_num(trials_df.contrastLeft) - np.nan_to_num(
                trials_df.contrastRight
            )
    ...
    elif (kwargs["target"] == "pLeft") and (
        kwargs["model"] is optimal_Bayesian
    ):  # bypass fitting and generate priors
    ...
    if (
        ...
        and (kwargs["model"] is not None)
        and kwargs["model_parameters"] is None
    ):
        ...
        for _ in metadata[
            "eids_train"
        ]:  # this seems superfluous but this is a relevant structure for when eids_train != [eid]
            side, stim, act, _ = format_data_mut(trials_df)
            datadict["stim_side"].append(side)
            datadict["stimuli"].append(stim)
            datadict["actions"].append(act)
        stimuli, actions, stim_side = format_input_mut(
            datadict["stimuli"], datadict["actions"], datadict["stim_side"]
        )
        model = kwargs["model"](
            kwargs["behfit_path"],
    ...
    if kwargs["model_parameters"] is None:
        signal = model.compute_signal(
            signal="prior" if kwargs["target"] == "pLeft" else kwargs["target"],
            act=actions,
            stim=stimuli,
            side=stim_side,
        )["prior" if kwargs["target"] == "pLeft" else kwargs["target"]]
    else:
        output = model.evaluate(
            np.array(list(kwargs["model_parameters"].values()))[None],
            return_details=True,
            act=actions,
            stim=stimuli,
            side=stim_side,
...
def get_target_data_per_trial(
    ...
    for i, (target_time, target_vals) in enumerate(
        zip(target_times_og_list, target_data_og_list)
    ):
...
def get_target_variable_in_df(
    one, eid, target, align_time, time_window, binsize, **kwargs
):
...
def check_bhv_fit_exists(
    subject, model, eids, resultpath, modeldispatcher, single_zeta
):
    """
    ...
    path_results_mouse = (
        "model_%s_" % modeldispatcher[model] + "single_zeta_" * single_zeta
    )
