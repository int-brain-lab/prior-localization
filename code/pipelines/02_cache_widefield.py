...
# IBL libraries
from code.params import CACHE_PATH, WIDE_FIELD_PATH
from code.wide_field_imaging.wfi_utils import load_wfi_session
...
hemisphere_specif = (
    "both_hemispheres" if np.unique(HEMISPHERES).size > 1 else HEMISPHERES[0]
)

subjects = glob.glob(WIDE_FIELD_PATH.joinpath("CSK-im-*").as_posix())
nb_eids_per_subject = np.array(
    [np.load(s + "/behavior.npy", allow_pickle=True).size for s in subjects]
)
...
for index in tqdm(range(nb_eids_per_subject.sum())):
    ...
    sessiondf, wideFieldImaging_dict, metadata = load_wfi_session(
        subjects[subj_id], sess_id, HEMISPHERES, WIDE_FIELD_PATH.as_posix()
    )

    sesspath = (
        Path(CACHE_PATH)
        .joinpath("widefield")
        .joinpath(metadata["subject"])
        .joinpath(metadata["eid"])
        .joinpath(hemisphere_specif)
    )
...
outdict = {
    "params": {"hemisphere_specif": hemisphere_specif},
