...
def load_wheel_data(one, eid, sampling_rate=1000):
    ...
    wheel_pos, wheel_times = interpolate_position(
        wheel_times_og, wheel_pos_og, freq=sampling_rate
    )
...
def load_pupil_data(one, eid, view="left", snr_thresh=5):
    ...
    if times.shape[0] != diam0.shape[0]:
        raise TimestampError
    pupil_diam = smooth_interpolate_savgol(
        diam0, window=31, order=3, interp_kind="linear"
    )
    good_idxs = np.where(~np.isnan(diam0) & ~np.isnan(pupil_diam))[0]
    snr = np.var(pupil_diam[good_idxs]) / np.var(
        diam0[good_idxs] - pupil_diam[good_idxs]
    )
    if snr < snr_thresh:
...
def load_paw_data(
    ...
    if jumps["r"] > n_jump_thresh:
        raise QCError(
            "number of large jumps (%i) is above threshold (%i)"
            % (jumps["r"], n_jump_thresh)
        )
...
def get_dlc_traces(one, eid, view, likelihood_thresh=0.9):
    ...
    for point in points:
        x = np.ma.masked_where(
            cam[point + "_likelihood"] < likelihood_thresh, cam[point + "_x"]
        )
        x = x.filled(np.nan)
        y = np.ma.masked_where(
            cam[point + "_likelihood"] < likelihood_thresh, cam[point + "_y"]
        )
        y = y.filled(np.nan)
