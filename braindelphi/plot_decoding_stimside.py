...
neurometric_curve = np.array([np.mean(preds[targ_conts==c]) for c in u_conts])
neurometric_curve_err = np.array([2*np.std(preds[targ_conts==c]) for c in u_conts])
plt.plot(-u_conts, 1-neurometric_curve, lw = 4, c='k')
plt.ylim(0,1)
plt.xlim(-1,1)
...
regions = regions[np.argwhere(regions=='MEA')[0][0]:]
