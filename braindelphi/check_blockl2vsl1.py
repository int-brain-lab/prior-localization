...
for s in zip(list(regs_data_old['frac_sig']), 
          list(regs_data['frac_sig']), 
          list(regs_data_old['n_sessions']),
          list(regs_data['n_sessions'])):
    if (not (np.isnan(s[0]) or np.isnan(s[1]))) and (not (s[0] == s[1])):
        plt.text(s[0],s[1], f'{s[2]}/{s[3]}', fontsize=6)
plt.xlabel('Frac Sig, \nL2 regularization')
plt.ylabel('Frac Sig, \nL1 regularization')
plt.xlim(-0.05, 1.05)
plt.ylim(-0.05, 1.05)
