    ...
    for eid in np.unique(res['eid']):
        ...
        for reg in np.unique(reseid['region']):
            ...
            if RETURN_X_Y:
                ...
                # check arrays
                print('my targs', my_targets.shape)
...
res_bcw = fix_pd_regions(res)

bcw_table = create_pdtable_from_raw(res_bcw, 
                            score_name='R2_test',
                            N_PSEUDO=100, N_RUN=2,
                            RETURN_X_Y=False,
                            SCALAR_PER_TRIAL=False,
                            SAVE_REGRESSORS=False)

#%%
print('Working on Wheel-Speed')
file_pre = 'decoding_results/legacy_data/27-10-2022_decode_wheel-speed_task_Lasso_align_firstMovement_times_100_pseudosessions_regionWise_timeWindow_-0_2_1_0_imposterSess_1_balancedWeight_0_RegionLevel_1_mergedProbes_1_behMouseLevelTraining_0_constrainNullSess_0_paraindex'
res = pd.DataFrame()
for i in range(50):
    res_new = pd.read_pickle(file_pre+str(i)+'.pkl')
    res = pd.concat([res, res_new], axis=0)
...
eids = np.array(bcw_table['eid'])
for eid in eids:
    bcweid_table = bcw_table.loc[bcw_table['eid']==eid]
    regs = np.array(bcweid_table['region'])
    for reg in regs:
    
        bcweidreg_table = bcweid_table[bcweid_table['region']==reg]
        assert bcweidreg_table.shape[0] == 1
        reseidreg_table = res_table[(res_table['eid']==eid)&(res_table['region']==reg)]
        assert reseidreg_table.shape[0] == 1
        res_nulls = reseidreg_table.iloc[0]['null-scores']
        bcw_nulls = bcweidreg_table.iloc[0]['null-scores']
        
        res_mu, res_sig = np.mean(res_nulls), np.std(res_nulls)
        bcw_mu, bcw_sig = np.mean(bcw_nulls), np.std(bcw_nulls)
        
        res_mus.append(res_mu)
        res_sigs.append(res_sig)
        bcw_mus.append(bcw_mu)
        bcw_sigs.append(bcw_sig)
        
plt.title('Mean of null scores')
plt.hist(np.array(res_mus)-np.array(bcw_mus), bins=15, histtype='step', lw=3)
#plt.hist(bcw_mus, bins=15, histtype='step', lw=3)
#plt.legend(['ephys', 'bias choice world'])
plt.show()
