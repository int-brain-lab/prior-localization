...
def sag_slice_RGB(list_region_id, list_region_value, ML_coordinate, color_range=1):
    ...
    mode='raise'
    mapping='Beryl'
    sag_slice_ind=ba._get_mapping(mapping=mapping)[_take(ba.label, index_1, ba.xyz2dims[axis])]
...
def ctx_slice_RGB(list_region_id, list_region_value,color_range=1):
...
def generate_sag_slices(list_region_label, 
                        list_region_value,
                        filename = 'test.svg',
    ...
    list_region_id = ba.regions.acronym2id(list_region_label)
...
def generate_ctx_top(list_region_label, list_region_value):
