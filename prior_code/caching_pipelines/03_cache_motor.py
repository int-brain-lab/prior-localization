...
def get_ME(eid, video_type, query_type="remote"):
    # video_type = 'left'
    Times = one.load_dataset(
        eid, f"alf/_ibl_{video_type}Camera.times.npy", query_type=query_type
    )
    ME = one.load_dataset(
        eid, f"alf/{video_type}Camera.ROIMotionEnergy.npy", query_type=query_type
    )
    return Times, ME
...
def get_dlc_XYs(eid, video_type, query_type="remote"):
    ...
    cam = one.load_dataset(
        eid, f"alf/_ibl_{video_type}Camera.dlc.pqt", query_type=query_type
    )
    points = np.unique(["_".join(x.split("_")[:-1]) for x in cam.keys()])
    # Set values to nan if likelyhood is too low # for pqt: .to_numpy()
    XYs = {}
    for point in points:
        x = np.ma.masked_where(cam[point + "_likelihood"] < 0.9, cam[point + "_x"])
        x = x.filled(np.nan)
        y = np.ma.masked_where(cam[point + "_likelihood"] < 0.9, cam[point + "_y"])
        y = y.filled(np.nan)
    ...
    for tr in range(len(trials["intervals"])):
        ...
        for be in behaves:
            ...
            if pawex and ("paw" in be):  # for illustration on frame
                D[be].append(
                    [series[0][start_idx:end_idx], series[1][start_idx:end_idx]]
                )
            else:
                # bug inducing
...
def load_motor(eid):
    ...
    motor_regressors = cut_behavior(
        eid, duration=2, lag=-1, query_type="auto"
    )  # very large interval
    ...
    regressors = dict(
        filter(lambda i: i[0] in motor_signals_of_interest, motor_regressors.items())
    )
    return regressors
...
def cache_motor(subject, eid, regressors):
    ...
    metadata = {"subject": subject, "eid": eid}
...
bwm_df = bwm_query().set_index(["subject", "eid"])  # frozen dataset
...
for eid in tqdm(bwm_df.index.unique(level="eid")):
    ...
    if eid not in motor_eids:
        continue
    try:
        load_outputs = load_motor(eid)
        saved_outputs = cache_motor(subject, eid, load_outputs)
        outlist.append([subject, eid, saved_outputs])
    except:
        failed_eids.append(eid)
        pass

print("number of failed eids is {}".format(len(failed_eids)))
...
dataset = [
    {
        "subject": x[0],
        "eid": x[1],
        "meta_file": outlist[i][-1][0],
        "reg_file": outlist[i][-1][1],
    }
    for i, x in enumerate(outlist)
]
