...
def load_wfi_session(
    path_to_wfi_subject, sessid, hemispheres, path_to_wfi, keep_all_session=False
):
    ...
    if (
        "left" not in hemispheres
    ):  # if 'left' is not in hemispheres, only keep regions with negative labels (right hem)
        ds_atlas = (ds_atlas < 0) * ds_atlas
    if (
        "right" not in hemispheres
    ):  # if 'right' is not in hemispheres, only keep regions with positive labels (left hem)
        ds_atlas = (ds_atlas > 0) * ds_atlas
    ...
    sessiondf["eid"] = sessiondf["eid"].apply(
        lambda x: ("wfi" + str(int(path_to_wfi_subject.split("-")[-1])) + "s" + str(x))
    )
    ...
    wideFieldImaging_dict = {
        ...
        "timings": frame_df,
        "regions": ds_atlas,
        "atlas": atlas[["acronym", "name", "label"]],
    }
...
def decompose_downsampled_files(path_to_wfi):
    downsampled = numpy.load(
        path_to_wfi + "/downsampled_ims-%s.npy" % path_to_wfi.split("-")[-1],
        allow_pickle=True,
    )
...
def format_resultsdf(
    ...
    df_sess_reg["corrected_R2_test"] = (
        df_sess_reg["R2_test"] - df_sess_reg["pseudo_R2_test"]
    )
    ...
    df_sess_reg["pseudo_distrib"] = (
        df_pseudo_sess[["R2_test", "eid"]].groupby("eid").agg(list)
    )

    df_sess_reg["tvalue"] = df_sess_reg.apply(
        lambda x: scipy.stats.ttest_1samp(x.pseudo_distrib, x.tvalue).statistic
    )
...
def select_widefield_imaging_regions(neural_dict, region, kwargs):
    ...
    region_labels = []
    reg_lab = neural_dict["atlas"][
        neural_dict["atlas"].acronym.isin(region).values
    ].label.values.squeeze()
