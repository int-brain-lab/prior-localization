...
import pickle
import pandas as pd
from brainbox.task.closed_loop import (
    generate_pseudo_blocks,
    _draw_position,
    _draw_contrast,
)
from prior_code.decoding.functions.nulldistributions import generate_imposter_session
...
import torch
from prior_code.decoding.settings import modeldispatcher
...
def balanced_weighting(
    vec, continuous, use_openturns, bin_size_kde, target_distribution
):
    ...
    if continuous:
        if use_openturns:
            ...
            emp_distribution = np.histogram(
                vec, bins=target_distribution[-1], density=True
            )
            balanced_weight = pdf_from_histogram(
                vec, target_distribution
            ) / pdf_from_histogram(vec, emp_distribution)
...
def get_target_pLeft(
    ...
    if subjModel is not None:
        ...
        if subjModel["model_parameters"] is None:
            ...
            if not istrained:
                raise ValueError(
                    "Something is wrong. The model should be trained by this line"
                )
            model.load_or_train(loadpath=str(fullpath))
    else:
        model = None
    contrast_set = np.array([0.0, 0.0625, 0.125, 0.25, 1])
    target_pLeft = []
    for _ in np.arange(nb_sessions):
        if model is None or not subjModel["use_imposter_session_for_balancing"]:
            ...
            pseudo_trials.loc[
                pseudo_trials["signed_contrast"].isnull(), "signed_contrast"
            ] = -pseudo_trials["contrastLeft"]
            pseudo_trials["choice"] = 1  # choice padding
        else:
            pseudo_trials = generate_imposter_session(
                subjModel["imposterdf"], subjModel["eid"], nb_trials, nbSampledSess=10
            )
        side, stim, act, _ = format_data_mut(pseudo_trials)
        if model is None:
            msub_pseudo_tvec = optimal_Bayesian(act, side)
        elif (
            not subjModel["use_imposter_session_for_balancing"]
            and model.name == modeldispatcher[expSmoothing_prevAction]
        ):
            arr_params = (
                model.get_parameters(parameter_type="posterior_mean")[None]
                if subjModel["model_parameters"] is None
            ...
            act_sim, stim, side, msub_pseudo_tvec = model.simulate(
                arr_params,
                stim,
                side,
            ...
            not subjModel["use_imposter_session_for_balancing"]
            and model.name == modeldispatcher[expSmoothing_stimside]
        ):
            arr_params = (
                model.get_parameters(parameter_type="posterior_mean")[None]
                if subjModel["model_parameters"] is None
            ...
            output = model.evaluate(
                arr_params[None], return_details=True, act=act, stim=stim, side=side
            )
            ...
            msub_pseudo_tvec = model.compute_signal(
                signal=(
                    "prior" if subjModel["target"] == "pLeft" else subjModel["target"]
                ),
                act=act,
                stim=stim,
                side=side,
            )
            msub_pseudo_tvec = msub_pseudo_tvec[
                "prior" if subjModel["target"] == "pLeft" else subjModel["target"]
            ]
            raise NotImplementedError(
                "this is not supported since merge with wheel velocity"
            )
    ...
    out = np.histogram(
        target_pLeft,
        bins=(
            np.arange(-bin_size_kde, 1 + bin_size_kde / 2.0, bin_size_kde)
            + bin_size_kde / 2.0
        ),
        density=True,
    )
...
def get_balanced_weighting(trials_df, metadata, **kwargs):
    if kwargs["balanced_weight"] and kwargs["balanced_continuous_target"]:
        if not kwargs["use_imposter_session_for_balancing"] and (
            kwargs["model"] == optimal_Bayesian
        ):
            target_distribution, _ = get_target_pLeft(
            ...
            subjModel = {
                ...
                "use_imposter_session_for_balancing": kwargs[
