...
from code.decoding.functions import utils as dut
...
def get_neural_activity(df_insertions, eid, one, wideFieldImaging_dict=None, **kwargs):
    ...
    trialsdf["react_times"] = (
        trialsdf["firstMovement_times"] - trialsdf[kwargs["align_time"]]
    )
    mask = (
        trialsdf[kwargs["align_time"]].notna() & trialsdf["firstMovement_times"].notna()
    )
    if kwargs["no_unbias"]:
    ...
    if kwargs["merged_probes"] and wideFieldImaging_dict is None:
        ...
        for i_probe, (_, ins) in tqdm(
            enumerate(df_insertions.iterrows()), desc="Probe: ", leave=False
        ):
            probe = ins["probe"]
            spike_sorting_path = Path(ins["session_path"]).joinpath(
                ins["spike_sorting"]
            )
            ...
            across_probes["clusters"].extend(
                spikes.clusters
                if i_probe == 0
                else (spikes.clusters + max(across_probes["clusters"]) + 1)
            )
        ...
        df_insertions_iterrows = pd.DataFrame.from_dict(
            {"1": "mergedProbes"}, orient="index", columns=["probe"]
        ).iterrows()
        ...
        for region in tqdm(regions, desc="Region: ", leave=False):
            if kwargs["merged_probes"] and wideFieldImaging_dict is None:
                ...
                reg_lab = wideFieldImaging_dict["atlas"][
                    wideFieldImaging_dict["atlas"].acronym == region
            ...
            if kwargs["merged_probes"] and wideFieldImaging_dict is None:
                ...
                binned, _ = get_spike_counts_in_bins(
                    regspikes[arg_sortedSpikeTimes],
                ...
                frames_idx = wideFieldImaging_dict["timings"][
                    kwargs["align_time"]
                ].values
                frames_idx = np.sort(
                    frames_idx[:, None]
                    + np.arange(
                        0, kwargs["wfi_nb_frames"], np.sign(kwargs["wfi_nb_frames"])
                    ),
                    axis=1,
                )
                binned = np.take(
                    wideFieldImaging_dict["activity"][:, reg_mask], frames_idx, axis=0
                )
