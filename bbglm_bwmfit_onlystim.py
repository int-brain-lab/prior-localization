...
def fit_session(session_id, kernlen, nbases,
                t_before=0.2, t_after=0., prior_estimate=None, max_len=2., probe_idx=0,
    ...
    stepbounds_stim = [nglm.binf(t_before), nglm.binf(t_before + kernlen)]

    def stepfunc_poststim(row):
        stepvec = np.zeros(nglm.binf(row.duration))
        stepvec[stepbounds_stim[0]:stepbounds_stim[1]] = row.pLeft_last
        return stepvec
    ...
    nglm.add_covariate_raw('pLeft', stepfunc_poststim, desc='Step function on prior estimate')
