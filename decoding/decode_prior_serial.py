...
for eid in eid_list:
    ...
    #if np.any(np.isnan(spikes[probe].times)) or \
    #        np.any(np.isnan(spikes[probe].clusters)) or np.any(np.isnan(trialsdf[ALIGN_TIME])):
    #    raise ValueError('NaN is the spike data')
...
pickle.dump(lite, open('sanity_analysis/summary_20sessions_func_get_spike_counts_in_bins.pkl', 'wb'))
