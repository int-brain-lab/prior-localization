...
def weibull50(pars, xx):
    ...
    if len(pars) != 3:
            raise Value ('pars must be of length 3')
...
def getpsych_rangeslopeshift(cc,nn,pp):
    pars, L = mle_fit_psycho(np.vstack((cc, nn, pp)), 'erf_psycho_2gammas')
    return erf_psycho_2gammas(pars, 1)-erf_psycho_2gammas(pars, -1), pars[1], erf_psycho_2gammas(pars, 0), lambda x: erf_psycho_2gammas(pars, x), pars

def psycho_fit(pred,target,block,contrasts):

    neurometric_curve = [np.mean(pred[(target == contrasts[ci]) & (block == 1)]) for ci in range(len(contrasts))]
    numbers = [len(np.nonzero((target == contrasts[ci]) & (block == 1))[0]) for ci in range(len(contrasts))]
    block1_range, block1_slope, block1_shift, f, _ = getpsych_rangeslopeshift(contrasts,numbers,neurometric_curve)

    neurometric_curve = [np.mean(pred[(target == contrasts[ci]) & (block == 0)]) for ci in range(len(contrasts))]
    numbers = [len(np.nonzero((target == contrasts[ci]) & (block == 0))[0]) for ci in range(len(contrasts))]
    block0_range, block0_slope, block0_shift, f, _ = getpsych_rangeslopeshift(contrasts,numbers,neurometric_curve)

    bshift = block1_shift-block0_shift
    brange = .5*(block1_range+block0_range)
    bslope = .5*(block1_slope+block0_slope)
    
    return brange, bslope, bshift